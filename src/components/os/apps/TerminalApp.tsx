...
const TerminalApp: React.FC = () => {
  ...
  return (
    <div
      className="h-full flex flex-col p-3 font-mono text-sm overflow-y-auto"
      style={{ background: darkMode ? 'rgba(20,20,22,0.7)' : 'rgba(245,245,247,0.75)', color: darkMode ? '#e8e8e8' : '#1d1d1f', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace', backdropFilter: 'blur(30px) saturate(180%)', WebkitBackdropFilter: 'blur(30px) saturate(180%)' }}
      onClick={() => inputRef.current?.focus()}
    >
      {lines.map((line, i) => (
        <div key={i} className="whitespace-pre-wrap break-all leading-relaxed" style={{ color: line.type === 'input' ? (darkMode ? '#64D2FF' : '#006EFC') : (darkMode ? '#e8e8e8' : '#1d1d1f') }}>
          {line.text}
        </div>
      ))}
      <div className="flex items-center">
        <span style={{ color: darkMode ? '#64D2FF' : '#006EFC' }}>{t.terminal.prompt} </span>
        <input
          ref={inputRef}
          type="text"
          value={input}
          ...
          style={{ color: darkMode ? '#e8e8e8' : '#1d1d1f', caretColor: darkMode ? '#e8e8e8' : '#1d1d1f' }}
          autoFocus
          spellCheck={false}
        />
      </div>
