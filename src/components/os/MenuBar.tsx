...
const MenuBar: React.FC = () => {
  ...
  const appleMenu = [
    ...
    { separator: true },
    { label: t.menu.sleep, action: () => setLockScreen(true) },
  ...
  return (
    <div
      ...
      style={{
        background: darkMode ? 'rgba(20, 20, 22, 0.55)' : 'rgba(230, 230, 235, 0.55)',
        backdropFilter: 'blur(30px) saturate(200%)',
        WebkitBackdropFilter: 'blur(30px) saturate(200%)',
        borderBottom: darkMode ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(0,0,0,0.08)',
        boxShadow: darkMode ? 'inset 0 1px 0 rgba(255,255,255,0.08)' : 'inset 0 1px 0 rgba(255,255,255,0.6)',
      }}
        ...
        {menus.map((menu) => (
          <div key={menu.label} className="relative">
            <button
              ...
              } ${menu.isBold ? 'font-semibold' : 'font-medium'}`}
              onClick={(e) => {
                e.stopPropagation();
            ...
            >
              {menu.isLogo ? (
                <Logo size={16} style={{ display: 'block' }} color={darkMode ? '#fff' : '#000'} />
              ) : (
                menu.title
              )}
            </button>
            {activeMenu === (menu.isLogo ? 'apple' : menu.label) && menu.items.length > 0 && (
              <div
                className="absolute top-6 left-0 rounded-xl shadow-2xl py-1 min-w-[220px] z-[10000]"
                style={{
                  background: darkMode ? 'rgba(30,30,35,0.75)' : 'rgba(240,240,245,0.75)',
                  backdropFilter: 'blur(40px) saturate(200%)',
                  WebkitBackdropFilter: 'blur(40px) saturate(200%)',
                  border: darkMode ? '1px solid rgba(255,255,255,0.15)' : '1px solid rgba(0,0,0,0.1)',
                  boxShadow: darkMode ? '0 20px 50px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.1)' : '0 20px 50px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.8)',
                }}
                ...
                {menu.items.map((item: any, i: number) =>
                  item.separator ? (
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between px-4 py-1 text-[13px] rounded-lg mx-0.5 transition-colors hover:bg-blue-500 hover:text-white"
